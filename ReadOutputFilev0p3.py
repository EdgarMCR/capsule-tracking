...
#    pathParameters= path[0:ss1] + dSlash + path[ss3+1:ss2]+'_Parameters.txt'
        ImageWContourFromAdpativeThresholding
...
#            print('FPS=%d' %FPS)
            ...
            if readP and len((entriesLine)) >=14:
                centerline = float(entriesLine[8].strip())
                width = float(entriesLine[9].strip())
                pPmm = float(entriesLine[10].strip())
                geoTJ = [float(entriesLine[11].strip()), float(entriesLine[12].strip()), float(entriesLine[13].strip())]
...
#            analyisTwoCapsules(path,centerline, width, pPmm, FPS=64, Index=None, borderSize=10, debugInfo=True, closeAfterPlotting=False, geoCutOff=[0,0,0], plot=True)
            findSpeedGradient(pathFolder,centerline=centerline, widthChannel=width, pPmm=pPmm, FPS=FPS, Index=Index, UseEverySecond=False, borderSize=10, closeAfterPlotting=True, geoCutOff=geoTJ, plot=False)
            plt.close('all')
            gc.collect()
...
def readResultsFile(path, second=False):
    ...
    width =np.zeros((numOfLines,1), dtype=float)
...
#==============================================================================
#==============================================================================
def findSpeedGradient(path,centerline, widthChannel, pPmm, FPS=64, Index=None, UseEverySecond=False, borderSize=10, debugInfo=True, closeAfterPlotting=False, geoCutOff=[161, 540, 720], plot=True):
    if debugInfo:
        print(Index)
...
#    print('M:\\EdgarHaener\\Capsules\\RigidParticle-3.5mm\\31032015\\Rigid3p5mm-310315-30fps-10mlPmin-1\\workfile.txt')   
    ...
    if not os.path.isfile(pathData): #check whether the file for this run has been started
        fileData = open(pathData, 'w')
        fileData.write('Vol Flux \t\t\t\t\t\tname \t min speed \t ave speed 1 \t ave speed 2 \t time acceleration  \ \t Off-centre \t turned \t time geometric \t max width \t max d12 \tdifferenceTimeStart \tdifferenceTimeEnd \td12 Main \t d12 Daugther\n')
        fileData.close()
    if not os.path.isfile(pathParameters):
        fileParameters = open(pathParameters, 'w')
        fileParameters.write('fileID \t\t\t\t\t\tshort \t threshold 1 \t threshold 2 \t Pos \tcenterline \twidth \tpPmm \tgeometryTJ\n')
        fileParameters.close()
    
    #check whether there is alread a value for this fileID
    stringWRslt = '%.1f \t %s \t \t \t %.2e \t %.2e \t %.2e  \t %.2f \t %.2f \t %s \t %.3e \t %d \t %.3f \t%.3e \t%.3e \t%.3f \t%.3f\n' %(volFlux, fileID, minSpeed, firstSpeed,  secondSpeed, timeInTJunction, differenceX, turn, timeTJ, maxwidth, maxd12, differenceTimeStart, differenceTimeEnd, d12Main, d12Daugther)
    if fileID in open(pathData).read():
         #File 1: Data
        f = open(pathData,"r")
        lines = f.readlines()
    ...
    if fileID in open(pathParameters).read():
        ...
        for line in lines:
            if fileID in line:
                if Index ==None:
                    fileParameters.write('%s \t  %.2e  \t %.2f \t %.2f \t %d \t %dcentroid_x, centroid_y, v_x, v_y, area, width, d12, height = readResultsFile(path, second=False) \t %d \t %d \t %f \t %f \t %f \t %d \t %d \t %d \n' %(fileID, short, startThres, endThres, -1, -1, -1, -1, centerline, widthChannel, pPmm, geoCutOff[0], geoCutOff[1], geoCutOff[2]))
                else:
                    fileParameters.write('%s \t  %.2e  \t %.2f \t %.2f \t %d \t %d \t %d \t %d \t %f \t %f \t %f \t %d \t %d \t %d \n' %(fileID, short, startThres, endThres, Index[0], Index[1], Index[2], Index[3], centerline, widthChannel, pPmm, geoCutOff[0], geoCutOff[1], geoCutOff[2]))
...
#    folder =  'Batch270715-001-#5-%dFPS-15mlPmin-4\\' %FPS
    ...
    folder =  'Batch260615-001-#17-%dFPS-50mlPmin-7\\' %FPS

#    directory = 'M:\\EdgarHaener\\Capsules\\Batch120615-004\\T-Junction\\'
#    folder =  'Batch120615-001-#4-%dFPS-50mlPmin-6\\' %FPS
...
#    geometryTJ=[122, 549,  722] #Batch040615-002
#    centerline, width, pPmm = 637, 175, 22.4; geometryTJ=[159, 550,  725] #Batch120615-004 #4 15ml/min
#    centerline, width, pPmm = 637, 175, 22.4; geometryTJ=[118, 549,  724] #Batch120615-004 #4 Other Runs
...
#    pos=[3,15, 90, 110]
#    pos=[10, 30, 100, 130]
    findSpeedGradient(path, centerline=centerline, widthChannel=width, pPmm=pPmm, FPS=FPS, Index=pos, UseEverySecond=False, borderSize=10, debugInfo=True, closeAfterPlotting=False,  geoCutOff=geometryTJ)
