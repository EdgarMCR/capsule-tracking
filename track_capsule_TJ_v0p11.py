...
#    print('Thresholding - Type img = ' +str(type(img)))
    ...
    while notFoundMatchingContour and counterWhile < len(contours):   
        ...
        #if longest contour was found, then ID is the index of it
        if ID != -1:
            if len(contours[ID]) > 50 and len(contours[ID]) < 350:
                cnt = contours[ID] 
                if expectedArea != None:
                    M = cv2.moments(cnt)
                                ...
                                if M['m00'] > minArea*expectedArea and M['m00'] < maxArea*expectedArea:
                                    ...
                                    if w < xImg*0.9 and h < yImg*0.9:
                                        if maxPerimeter > perimeter:
                                            print('Winning parameter combination: threshold = %d ratio= %.1f kernel=%d convexHull = %r median blur =%d A/Ae = %.2f len(cnt) = %d' %(lt, r, k, False, m, M['m00']/expectedArea, len(cnt)))
                                            return cnt, x, y, w, h
                                else:
...
#    threshold=150
    ...
    back = cv2.imread(path[:d]+'\\Background.png',0)
...
#        print("readfind_max_extendPath: %s" %readPath)
        ...
        imgC=cv2.copyMakeBorder(imgC,borderSize,borderSize,borderSize,borderSize,
                               cv2.BORDER_CONSTANT,value=(255, 255, 255))
    ...
    #            top= topCropLevel
            ...
            if plot:
                if platform.system() == 'Linux':
                    ...
                    savepath=path+'Check/'
                if not os.path.exists(savepath): os.makedirs(savepath)
                cv2.imwrite(path+'Check\\Img-BackgroundSubtracted_'+str(i)+'.jpg', imgC)
    
            imgForCanny = img[top:yBT+h+padding, xBT-padding:xBT+w+padding]
#            print('imgForCanny = (%d, %d)' %imgForCanny.shape)
