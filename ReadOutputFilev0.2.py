...
def rerunWithOldParameters(directory,centerline, width, pPmm, geoTJ):    
...
#    print('ss2 = %s \t %s' %(ss2, directory[:ss2]))
    ...
    if  not os.path.isfile(pathDataSave):
...
#            print('FPS=%d' %FPS)
            ...
            findSpeedGradient(pathFolder,centerline=centerline, width=width, pPmm=pPmm, FPS=FPS, Index=Index, UseEverySecond=False, borderSize=10, closeAfterPlotting=True, geoCutOff=geoTJ, plot=False)
            plt.close('all')
            gc.collect()
...
def readResultsFile(path):
    ...
    return centroid_x, centroid_y, v_x, v_y, area, width, d12
    
def plotCentorid(path, numberToPlot=3, borderSize=10, rotate=0, show=False):
    
    centroid_x, centroid_y, _, _, _, _, _ = readResultsFile(path) 
...
def findSpeedGradient(path,centerline, width, pPmm, FPS=64, Index=None, UseEverySecond=False, borderSize=10, debugInfo=True, closeAfterPlotting=False, geoCutOff=[161, 540, 720], plot=True):
    if debugInfo:
        print(Index)
    ...
    fileID=tr.find_batchName(path);
    centroid_x, centroid_y, v_x, v_y, area, width, d12 = readResultsFile(path)  
...
#    print('Name: %s \nVolumn Flux = %s' %(name, volFlux))
    ...
    print('Vol Flux \tname \t min speed \t ave speed 1 \t ave speed 2 \t time \t short \t threshold 1 \t threshold 2 \t Off-centre \t turned \t max width \t max d12 \t differenceTimeStart \t differenceTimeEnd' )
    print('%.0f \t %s \t %.2e \t %.2e \t %.2e \t %.2e \t %.2f \t %.2f \t %.2f \t %.2f \t %s \t %.3e \t %d \t %.3f \t %.3e \t%.3e' %(volFlux, fileID, minSpeed, firstSpeed,  secondSpeed, timeInTJunction, short, startThres, endThres, differenceX, turn, timeTJ, maxwidth, maxd12, differenceTimeStart, differenceTimeEnd ))           
...
#    print('M:\\EdgarHaener\\Capsules\\RigidParticle-3.5mm\\31032015\\Rigid3p5mm-310315-30fps-10mlPmin-1\\workfile.txt')   
    ...
    if not os.path.isfile(pathData): #check whether the file for this run has been started
        ...
        fileData.write('Vol Flux \t\t\t\t\t\tname \t min speed \t ave speed 1 \t ave speed 2 \t time acceleration  \ \t Off-centre \t turned \t time geometric \t max width \t max d12\n')
        fileData.close()
    ...
    if fileID in open(pathData).read():
        ...
        for line in lines:
            if fileID in line:
                fileData.write('%.0f \t %s \t \t \t %.2e \t %.2e \t %.2e  \t %.2f \t %.2f \t %s \t %.3e \t %d \t %.3f \t%.3e \t%.3e\n' %(volFlux, fileID, minSpeed, firstSpeed,  secondSpeed, timeInTJunction, differenceX, turn, timeTJ, maxwidth, maxd12, differenceTimeStart, differenceTimeEnd))
            else:
                fileData.write(line)        
...
#    path=directory + 'Batch170615-002-#2-100FPS-50mlPmin-4\\'
    ...
    path = directory + 'Batch260615-001-#17-70FPS-35mlPmin-2\\'

    FPS=70
    centerline=646 #629
    width=174
    pPmm=22.3
    geometryTJ=[114, 559, 733]
#    print('\npPmm = %f' %pPmm)
    pos=None
...
#    wholeRun(directory, centerline, width, pPmm, Index=None, UseEverySecond=False, geometryTJ=[160, 542, 718])
    pos=[10, 40, 120, 140]
#    pos=[1, 20, 60, 80]
    findSpeedGradient(path, centerline=centerline, width=width, pPmm=pPmm, FPS=FPS, Index=pos, UseEverySecond=False, borderSize=10, debugInfo=True, closeAfterPlotting=False, geoCutOff=geometryTJ)
